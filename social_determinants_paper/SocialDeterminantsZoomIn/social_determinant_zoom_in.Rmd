...
date_of_all = format(seq(as.Date("2020-03-23"), as.Date(strptime(date_of_study,"%m-%d-%Y")), by = "month"),"%m-%d-%Y")
...
save(covid_us_hist, file='covid_us_hist.rda')
...
if (state.abbr != "US") {
    covid.mort.mat <- dplyr::filter(covid_us_hist, state_abbr == state.abbr) %>%
...
county.clusters <- km.func(covid.mort.mat, cluster.num=3)
cluster.deathrates <- get.cluster.deathrate.during.time(county.clusters, covid_us_hist, death.cause)
cluster.order.map <- get.cluster.order.map(cluster.deathrates, time.period = "09-23-2020")
...
```{r}
print(kendall.cor$chr_code)
```
```{r}
selected.determinant <- kendall.cor$chr_code[9]
...
cluster.determinant.vals.deathrate <- dplyr::inner_join(cluster.determinant.vals, 
                                              cdc.mort.mat(covid_us_hist, state, death.cause
                                                            )[c("county_fips", "09-23-2020")],
                                              by = "county_fips")

colnames(cluster.determinant.vals.deathrate)[colnames(cluster.determinant.vals.deathrate)=="09-23-2020"] <- "mortality_rate"
